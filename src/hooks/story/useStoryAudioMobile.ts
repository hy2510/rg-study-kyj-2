...
export default function useStoryAudioMobile(pageData: PageProps[]) {
  ...
  useEffect(() => {
    ...
    const handlerEnded = () => {
      ...
      if (isAutoNextPage) {
        // 자동으로 넘기는 경우
        const isNextPage = pageData.find((data) => data.Page === pageNumber + 1)
      ...
      // 2. 세로인 경우 - 1페이지씩 진행
      if (pageData[pageData.length - 1].Page >= pageNumber + 1) {
        // 다음 페이지가 있는 경우
        if (isAutoNextPage) {
